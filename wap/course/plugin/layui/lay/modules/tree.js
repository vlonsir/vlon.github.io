/** layui-v2.5.5 MIT License By https://www.layui.com */
 ;layui.define("form",function(e){
	...
	b.prototype.render=function(){
		var e=this,a=e.config;e.checkids=[];
				...
				h=i(['<div data-id="'+r.id+'" class="layui-tree-set'+(r.spread?" layui-tree-spread":"")+(r.checked?" layui-tree-checkedFirst":"")+'">',
						'<div class="layui-tree-entry">',
							...
							function(){
								if(!t.edit)return"";
								var e={
									add:'<i class="layui-icon layui-icon-add-1" data-type="add" title="添加子菜单"></i>',
									update:'<i class="layui-icon layui-icon-edit" data-type="update" title="编辑菜单"></i>',
									del:'<i class="layui-icon layui-icon-delete" data-type="del" title="删除菜单"></i>'
	...
	b.prototype.spread=function(e,a){
		...
		k.on("click",function(){
		//点击左侧导航树 中的菜单文字  的事件
		//console.log("===========================点击左侧导航树 中的菜单文字  的事件===============================");	
			var n=i(this);
	...
	b.prototype.operate=function(e,a){
		...
		l.children(".layui-tree-btnGroup").on("click",".layui-icon",function(l){
			layui.stope(l);
			var f=i(this).data("type"),b=e.children("."+v),g={
				data:a,
				type:f,
				elem:e
			};
			...
			if("add"==f){
				datalist.push({
					parentId: a.id,
					id: ++treeTableAddRowId+"",
					name: "未命名",
					code: "",
					type: "",
	    			icon: "",
	    			level: parseInt(a.level)+1,
	    			upIds: (a.upIds||"")+"_"+a.id+"_",
	    			children: []
				});
				renderTreeInit();
				//console.log("====================点+号添加事件==============================");
				...
				//console.log("====================点击编辑事件==============================");
			 	...
			 	d.children(".layui-tree-editInput").blur(
			 		function(){
			 			//console.log("======================左侧编辑框失去焦点事件=============================");
			...
			//左侧删除节点事件 
				...
				if(a && a.children && a.children.length>0) {
					n.alert('请先删除该菜单下的子菜单', {title:"注意", icon: 3, btn: ['知道了']});
				} else {
					var aaa = a;
					n.confirm('确认删除菜单 "<span style="color: #999;">'+(a.title||"")+'</span>" 吗？此操作不能撤销！',{title:"注意"},function(a){
		 				//console.log("=================================左侧删除节点事件============================");
		 				...
		 				if(e.siblings("."+s).children("."+p)[0]){
							...
							if(r.showLine){
								...
								1==h?(b[0]||(
									...
									e.next()[0]||e.parents("."+s)[1]||e.parents("."+s).eq(0).next()[0]||e.prev("."+s).addClass(k)
						...
						e.remove();
						datalist = datalist.filter(i => i.id!=aaa.id) || [];
						renderTreeInit();
					})
				}
			}
